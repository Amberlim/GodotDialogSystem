...
# File paths
export(String, DIR) var portraits_folder
...
func play(play_node: String):
	...
	if current_dict.has("text"):
		text.bbcode_text = current_dict["text"]
	...
	if current_dict.has("go to"):
		if final_go_to_option_index.empty():
...
func continue_dialog(next_node_index_to_play, node_type = "Default"):
	...
	continue_button = continue_button.instance()
	...
	if node_type == "Default":
		...
		for node in next_node_index_to_play:
			...
			option.connect("option_pressed", self, "go_to_selected_node")
